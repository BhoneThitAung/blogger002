...
router.get('/',auth, function(req, res, next) {
  Post.countDocuments({author:req.session.user.id}, function(err,rtn){ //author total
    if(err) throw err;
    Comment.countDocuments({commenter: req.session.user.id}, function(err2, rtn2){ //commenter total
      if(err2) throw err2;
      Comment.countDocuments({author:req.session.user.id}, function(err3, rtn3){ //author comment total
        if (err3) throw err3;
        User.findById(req.session.user.id).select("favoriteB").exec(function(err4, rtn4){ // favblogger total
          if(err4) throw err4;
